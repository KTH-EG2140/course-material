...
*EG2140 · **self-paced** — do it with your lab partner during the week; your pod is first support (partner → pod → Discussions → the TA sessions) · ~110 min · host repo: the partner whose name comes LAST alphabetically in today's pair (so the other of you hosts this time) — both of you can push to it, every student has push on every workbook repo. No AI tools.*
...
1. **Branch out.** Each partner, from an up-to-date `main` (`git switch main && git pull` first — `switch -c` creates the branch and moves you onto it, as in LC4 Part A):
   ...
   - Partner B: `git switch -c feature/result-export`. Add `--out FILE` to the `pf` command: an `add_argument("--out", default=None, help=...)` next to `--scaling` in `main()`, and in `cmd_pf`, after the power flow, `net.res_line.to_csv(args.out)` when `args.out` is set — **and** add `| written to <file>` to the end of the same summary line.
...
```
   ...
   `HEAD` is the branch you are standing on (B's export), the part after `=======` is what came from `main` (A's scaling). Same question as LC4 Part B: which truth wins? **Both.** Edit the region into one print statement that carries the scaling factor *and* the file name, keep the `to_csv` lines, delete the three marker lines, then:

```bash
git add src/svedala_toolbox/cli.py
git commit -m "Merge main into feature/result-export: keep scaling in the summary and the --out export"
git push
```
...
**Checkpoint:** `git log --oneline --graph` shows the diamond — two branches leaving `main` and coming back:

```
*   912f16e Merge main into feature/result-export: keep scaling in the summary and the --out export
|\
| * a314634 pf: reject non-positive --scaling, show the factor in the summary
* | cb16ec9 pf: --out writes line results to CSV
|/
* 4de2c17 Lab 1 state
```
...
```
   ...
   `HEAD` is the newest commit (bad); `HEAD~11` is eleven commits earlier — "Document review note 1", the second commit, where the tests passed. `bisect run` checks out the middle commit, runs the command, reads pass or fail from its exit code, and repeats. Trimmed output on the reference run:
...
```
   ...
   Four test runs instead of eleven, and the same commit your hand search found. Now read what the "tidy formatting" commit actually did:
