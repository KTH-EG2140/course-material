...
*EG2140 · Quiz 1 at the scheduled sitting (30 min, individual, supervised); the room then stays open as open hours. The bug hunt itself is **self-paced** — do it with your lab partner during the week, pod as first support. No AI tools.*
...
Each pair receives **one script**: a short Svedala security summary that imports your own toolbox. It runs without crashing. It prints plausible numbers. **Exactly one of them is wrong, because the script contains one planted bug.** These bugs were written by an AI — in four weeks you will be reviewing its code daily; consider this a first taste of the genre: confident, tidy, wrong.
...
Take the clean script, plant **one** bug of your own — subtle, plausible, and wrong in a way a test could catch — and hand it to the pair on your left. Best planted bug of the day gets named in the next lecturecise.

## Done when

Test-fail-fix-pass committed; your planted bug delivered. That closes Module 1: you now have a tested, versioned, reviewed package built entirely by hand — and, more importantly, the reading habits to judge code you did *not* write. From Module 2 the toolbox starts eating real data.
