...
Implement `build_security_dataset(net_loader, year_parquet, n_hours=250, seed=7)` in `security.py`. `net_loader` is a *function* — pass `load_svedala` itself, without parentheses, and the function calls it to get a fresh network; `year_parquet` is the path of the parquet, read once with `pd.read_parquet`; `seed` pins the random sample of hours so a rerun gives the same table. Two constants at the top of the module name the columns: `ZONES = ["ZON_NORR", "ZON_MITT", "ZON_SYDVÄST", "ZON_EXTERN"]` and `TEMPS = ["temp_north", "temp_mid", "temp_south"]` — the same strings in the parquet and on your buses, `Ä` included. Three copies taken once, before the loop: `base_p`, `base_q` (loads) and `base_gp` (`net.gen["p_mw"].copy()`). Then step 1 once, steps 2–4 per sampled hour:
...
ds = build_security_dataset(load_svedala, "../data/svedala-year/svedala_hourly.parquet", n_hours=250, seed=7)   # the copy in your workbook
...
(`flagged` is the share of hours the rule calls insecure, `(proba >= threshold).mean()`.)
