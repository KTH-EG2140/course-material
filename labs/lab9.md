...
Module 3 gave you learners, quantiles, trees, networks and — above all — the evaluation discipline. This lab pools the class: each pair implements **one** challenger method against its own Lab 7 SARIMA baseline, on the **same harness**, and posts the result to the shared table. At the start of Lecturecise 15 we read the table together: what did each step of sophistication buy, and what did it cost?

## 1. Choose your challenger (~10 min)
One per pair, claimed first-come in the Discussions thread ("Lab 9 — method claims") so the table gets breadth: gradient boosting with temperature; linear regression on the LC11 features; quantile GBM (report pinball + coverage); MLP; SARIMA with temperature as exogenous input; or a method of your own (clear it in the thread).
...
## Done when
Your row is in the table with a working link, pod check exchanged. The table is discussed at Lecturecise 15 — arrive having read all of it.
