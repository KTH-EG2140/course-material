...
Log the unmapped ones with `logging` at level `INFO` — LC3 B4's two lines at the top of the module, `import logging` and `log = logging.getLogger(__name__)`, then one call after the loop: `log.info("zone map: %d loads mapped, %d unmapped: %s", n_mapped, len(unmapped), unmapped)` (the `%d` and `%s` are filled from the arguments, the logging module's own way of formatting). A module never configures logging, which means the line is **silent by default**; you see it only when the program that calls you switches it on, as LC3 B4 did with `logging.basicConfig(level=logging.INFO)`. On the reference solution, with INFO switched on, the line reads (the full list of 18 names, shortened here):
...
`source` may be a path *or* the bytes `zipfile` hands you in section 3; `bytes` needs `.decode("utf-8")`, a path needs `.read_text(encoding="utf-8")` — check with `isinstance(source, bytes)`. Look zones up with `zone_map.get(mrid, "unmapped")`, so an mRID the map has never seen lands in the unmapped bucket instead of raising a `KeyError` (this data has none, the next file might). One more observation you may have made: `loads.csv` also carries the mRID directly, in its `origin_id` column — a valid shortcut past the name step; the name route is the one LC8 showed. Either is fine, say which in the README.
...
From here on the checkpoints are Python, not shell, and they need `zone_map` built first — run them in a scratch notebook, `notebooks/lab6_explore.ipynb` in the host repo, committed (a plain script works too, but a plot in a script needs `plt.savefig`). Its first cell builds the map once:

```python
import pandas as pd
from svedala_toolbox.cim import load_zone_map, parse_ssh, assemble_series
CM = "<cm>"                                             # the path to your course-material clone
zone_map = load_zone_map("data/svedala-cim/network_EQ.xml", "data/svedala")
```

**Checkpoint:** `parse_ssh(f"{CM}/data/svedala-cim/network_SSH.xml", zone_map)` on the reference solution returns
...
**Checkpoint** on the reference solution:

```python
week = assemble_series(f"{CM}/data/svedala-year/ssh_week.zip", zone_map)
print(week.shape, week.index[0], "->", week.index[-1])
print(week.head(2).round(1))
```
...
168 hours, UTC, the five columns your policy produced (four if you dropped the unmapped loads — then say so). If a column hides behind `...`, that is pandas fitting the display, not a missing column — `pd.set_option("display.max_columns", 20)` shows them all. Plot it, the four zones only:
...
total_week, total_ref = week[zones].sum(axis=1), ref[zones].sum(axis=1)   # one total per hour
print(round(((total_week - total_ref) / total_ref).mean() * 100, 2), "% mapped zones vs the parquet")
print(round(((total_week + week["unmapped"] - total_ref) / total_ref).mean() * 100, 2), "% with the unmapped added back")
```

`.loc[week.index]` picks the reference rows at exactly your timestamps — both indexes are UTC, so they align. On the reference solution the per-zone mean deviation is 0.00 % for three zones and −0.76 % for `ZON_MITT` (a `-0.00` is a tiny negative number rounded, not a fault), the largest single-hour deviation 0.76 %, and the total over the four zones runs 0.42 % below the parquet — which is almost exactly the unmapped column: add it back and the total agrees to 0.12 %. **The residual is your unmapped loads**, and they sit in `ZON_MITT`. State the number and its explanation in your README — a comparison without a tolerance is not a comparison; start from 2 % per zone and tighten to what you measured.
...
(`writestr` writes bytes into the new archive under the given name; `ZIP_DEFLATED` compresses — about 380 kB for three files.) Paths inside the tests come from `__file__`, as in Lab 5: `Path(__file__).parent / "data" / "ssh_mini.zip"` for the zip, `Path(__file__).resolve().parents[1] / "data" / "svedala"` for the CSV folder, and `Path(__file__).resolve().parents[1] / "data" / "svedala-cim" / "network_EQ.xml"` for the EQ file you committed — `parents[1]` is two folders up from the test file, `tests/` then the repo root, the same idea as the loader's `parents[2]`. The shape test asserts three rows (`len(df) == 3`), a UTC index (`str(df.index.tz) == "UTC"`), and the four `ZON_*` columns present (`{"ZON_NORR", ...} <= set(df.columns)` — *is a subset of*).
