...
3. **Lift the magic.** The hard-coded path becomes a `DATA_DIR` constant and a function parameter with a default. (The after-file uses `pathlib.Path` — a standard-library object that represents a path, so `DATA_DIR / "loads.csv"` reads as *the file inside that folder* and works unchanged on Windows and macOS. It is a convenience, not a requirement — a plain string works here too, and the after-file accepts both by normalising with `Path(data_dir)` at the top of the function.)
