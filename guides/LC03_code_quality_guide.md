...
3. **Lift the magic.** The hard-coded path becomes a `DATA_DIR` constant and a function parameter with a default. (The after-file uses `pathlib.Path` — a standard-library object that represents a path, so `DATA_DIR / "loads.csv"` reads as *the file inside that folder* and works unchanged on Windows and macOS. It is a convenience, not a requirement — a plain string works here too, and the after-file accepts both by normalising with `Path(data_dir)` at the top of the function.) One honest label: `Path("data/svedala")` is relative to *where you run the script*, which is fine for a demo started from the repo root. Your workbook's `loader.py` builds its `DATA_DIR` from `__file__`, the module's own location, so the package finds its data from anywhere — the CLI, pytest, a notebook. The comment above that line explains the pieces; the two forms are the same idea with different anchors.
